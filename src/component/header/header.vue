...
.header-container
  .header
    .header-left
      span 欢迎登录{{theme.application}}
      span {{userInfo.full_name}}
      span 您好
    .header-right
      .header-left-logo(@click='goHomepage')
      div(:class="naviSelectedPath === paths[0] ? 'header-navi-selected' : 'header-navi'", @click='click(paths[0])') {{theme.store}}
      div(:class="naviSelectedPath === paths[1] ? 'header-navi-selected' : 'header-navi'", @click='click(paths[1])') 零件搜索
      ...
      div(:class="inperson ? 'header-navi-selected' : 'header-navi'", @click='click(paths[7])')
        el-dropdown(:show-timeout='0', :hide-timeout='0', @command='handlePersonCommand')
          span.el-dropdown-link 我的{{theme.application}}
            i.el-icon-arrow-down.el-icon--right
          el-dropdown-menu(slot='dropdown')
